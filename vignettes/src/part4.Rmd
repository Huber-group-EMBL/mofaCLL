...
#write_csv2(tRes, "sourceData_3b.csv")
```
...
corRecurr <- plotCor(plotTab, "LF4", "n.recurr", "CLL-PD", "Number of recurrent aberrations",
        "",colList[[5]]) 
corRecurr
#write_csv2(plotTab, "sourceData_3c.csv")
```
...
#write_csv2(plotTab, "sourceData_3e.csv")
plotMeanBeta
```
...
plotCpG <- ggplot(plotTab, aes(x=treat, y = CD19posKI67pos)) + #geom_boxplot(outlier.alpha = NA) + 
  ...
  geom_text(data =annoText, aes(label = tt), x = 1.5, y =40, vjust=-1, parse=TRUE) +
...
### Main Figure 3
```{r figure3, fig.height=9, fig.width=16}
...
pout
#ggsave("figure3.pdf",height = 9, width = 16)
```
...
```{r mutLoad_LF, fig.height=6, fig.width=12}
