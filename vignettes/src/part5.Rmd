...
**Heatmap of significantly correalted genes (1% FDR)**
...
rnaHeatmap <- pheatmap(plotMat, scale = "none", cluster_cols = FALSE, cluster_rows = FALSE,
         annotation_col = colAnno,
         color = colorRampPalette(c(colList[2],"white",colList[1]))(length(breaks)),
         breaks = breaks,
         show_rownames = FALSE, show_colnames = FALSE, silent = TRUE)$gtable
plot_grid(rnaHeatmap)
```
...
#write_csv2(as_tibble(enrichRes$enrichTab,rownames = "set"), "sourceData_4a.csv")
```
...
enrichRes.U <- runCamera(exprMat.U, designMat.U, gmts$H, 
                       ...
                       method = "camera", pCut = 0.05, ifFDR = TRUE, removePrefix = "HALLMARK_",
                       plotTitle = "Hallmarks (U-CLL, 5% FDR)")
...
enrichRes.M <- runCamera(exprMat.M, designMat.M, gmts$H, 
                       ...
                       method = "camera", pCut = 0.05, ifFDR = TRUE, removePrefix = "HALLMARK_",
                       plotTitle = "Hallmarks (M-CLL, 5% FDR)")
...
```{r heatmapMTOR, fig.height=13, fig.width=11}
...
plotSetHeatmapComplex(geneSigTab = corRes.sig, geneSet =gmts$H, setName = "HALLMARK_MTORC1_SIGNALING", exprMat, colAnno, scale = TRUE,
                      annoCol = annoColor, highLight =  highLightList)
```

**OXIDATIVE_PHOSPHORYLATION**
```{r heatmapOXPHOS,fig.height=15, fig.width=11}

plotSetHeatmapComplex(geneSigTab = corRes.sig, geneSet =gmts$H, setName = "HALLMARK_OXIDATIVE_PHOSPHORYLATION", exprMat, colAnno, scale = TRUE, annoCol = annoColor, highLight =  highLightList)
...
#write_csv2(nodeTab, "sourceData_4d.csv")
```
...
nodeList <- data.frame(id = seq(length(allNodes))-1, name = allNodes, stringsAsFactors = FALSE) %>%
  ...
  mutate(textSize = ifelse(type == "set",1,0.8))
...
enrichNet <- ggraph(tidyNet, layout = "igraph", algorithm = "nicely") + 
  ...
  geom_node_point(aes(color =type), size=3, alpha=0.5) + 
  geom_node_text(aes(label = pathText, col = type, size = type), repel = TRUE, fontface= "bold") +
  scale_size_manual(values =c(set = 6, yes = 5, no =4))+
  scale_color_manual(values = c(yes = colList[4],no = "grey60", set = colList[1])) + 
  #scale_edge_color_brewer(palette = "Set2") +
  theme_graph(base_family = "sans") + theme(legend.position = "none", plot.margin =margin(-10,0,-10,-10)) 
enrichNet
```
...
legP <- ggplot(legTab, aes(x=x, y=y, col = group)) +
  ...
  scale_color_manual(values = c("mitochondrial protein coding genes" = colList[4], "other genes" = "grey60"), name = "") +
...
enrichRes <- runCamera(exprMat, designMat, gmts$H, id = fData(gse.vst)$GENE_SYMBOL,
                       ...
                       plotTitle = "CpG ODN treatment", insideLegend = TRUE, setToHighlight = highSet)
...
gse.all <- gse.all[fData(gse.all)$`Gene Symbol`!="",]
...
enrichRes <- runCamera(exprMat, designMat, gmts$H, id = fData(gse.all)$`Gene Symbol`,
                       ...
                       method = "camera", pCut = 0.01, ifFDR = TRUE, removePrefix = "HALLMARK_",
                       plotTitle = "IL21 + CD40L treatment", insideLegend = TRUE, setToHighlight = highSet)
...
enrichRes <- runCamera(exprMat, mm, gmts$H, id = fData(gse.all)$`Gene Symbol`, contrast = "treatmentTcell",
                       method = "camera", pCut = 0.01, ifFDR = TRUE, removePrefix = "HALLMARK_",
                       plotTitle = "Co-culture with activated T cells", insideLegend = TRUE, setToHighlight = highSet)
...
enrichRes <- runCamera(exprMat, designMat, gmts$H, id = fData(gse.test)$`Gene Symbol`,
                       method = "camera", pCut = 0.01, ifFDR = TRUE, removePrefix = "HALLMARK_",
                       plotTitle = "anti-IgM treatment",insideLegend = TRUE, setToHighlight = highSet)
plotEnrich.IgM <- enrichRes$enrichPlot+ theme(plot.title = element_text(size=18, face = "bold"))
plotEnrich.IgM
```

## Enrichment analysis using all avboe stimulation signatures
...
##write_csv2(as_tibble(enrichRes$enrichTab,rownames = "set"), "sourceData_4b.csv")
```
...
plotProtMYC <- plotSetHeatmapComplex(geneSigTab = corRes.sigUp, geneSet =mycTar, setName = "HALLMARK_MYC_TARGETS", exprMat = protMat,colAnno =colAnno, scale = TRUE, annoCol = annoColor)
...
### MYC tagets
```{r protMycTarScatter,fig.height=5, fig.width=15}
...
plotProtScatter <- lapply(seleProt, function(n) {
  ...
  ggplot(eachTab, aes(x=LF4, y=value)) +
    geom_point(fill = colList[4],size=2,shape=21) + geom_smooth(method="lm",se=FALSE, linetype = "dashed", color = "grey50") +
...
Heatmap of significantly associated proteins (10% FDR), with mitochondrial proteins highlighed
...
```{r CLLPD_mitoProt,fig.height=6, fig.width=12}
...
plotMitoProScatter <- lapply(seleProt, function(n) {
  ...
  annoP <- paste("italic(P)~'=",formatNum(res$p.value, digits = 1, format = "e"),"'")
  ggplot(eachTab, aes(x=LF4, y=value)) +
    geom_point(fill = colList[4],size=3,shape=21) + geom_smooth(method="lm",se=FALSE, linetype = "dashed", color = "grey50") +
    annotate("text", x = -1.5, y = Inf, label = annoCoef,
           hjust=0, vjust =1, size = 5, col= colList[1]) +
    annotate("text", x = -1.5, y = Inf, label = annoP,
           hjust=0, vjust =3, size = 5, col= colList[1], parse= TRUE) +
...
```{r mitoTracker, fig.height=6, fig.width=6}
...
pList <- lapply(c("MitoTracker"), function(n) {
  eachTab <- filter(mitoTab, feature == n) 
  res <- cor.test(eachTab$logMFI, eachTab$CLLPD)
  annoCoef <- paste("coefficient =",format(res$estimate,digits = 2))
  annoP <- paste("italic(P)~'=",formatNum(res$p.value, digits = 2, format = "e"),"'")
  ggplot(eachTab, aes(x=CLLPD, y=logMFI)) +
...
plotMitoScatter <- grid.arrange(grobs= pList, ncol=1)
plotMitoScatter
```
...
```{r seahorseScatter,fig.height=5, fig.width=15}
...
## Loadings of drug view on LF1 and LF4

```{r, fig.width=5, fig.height=8}
plotLF1drug <- plotTopWeights.m(weightTab, view =  "Drugs", factor = "LF1", nfeatures = 15)
plotLF1drug
```

```{r,fig.width=5, fig.height=8}
plotLF4Drug <- plotTopWeights.m(weightTab, view =  "Drugs", factor = "LF4", nfeatures = 15)
plotLF4Drug
```
...
#ggsave("Figure4.png", height = 8, width = 12.5)
```

## Supplementary figures

### Enrichment for M-CLL and U-CLL separately
```{r enrichUM, fig.height=9, fig.width=10}
plot_grid(plotEnrichHallmark.U, NULL, plotEnrichHallmark.M, rel_heights = c(0.5,0.05,0.5), nrow = 3, align = "v")
```


### Asseble supplementary figures to show enrichment barplot
```{r otherStimulationErich,fig.height=15, fig.width=20, dev=c("png", "pdf")}
...
#pdf("test.pdf", height=14, width = 12)
...
plot_grid(grobHeatmap,
          plot_grid(plotMitoScatter, corMitoMarer, nrow=1, rel_widths = c(0.5,1), labels = c("B","C"), label_size = 22),
          ncol=1, rel_heights = c(3,1), labels = c("A",""), label_size = 22)
#ggsave("test.pdf", height = 18, width = 14)
```
