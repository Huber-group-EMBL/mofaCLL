...
Download and load CyTOF data on EMLB server
```{r,eval=TRUE}
...
#ggsave("proliferation_pop.pdf", height = 4, width = 9)
```
...
```{r}
exprTab <- getMedian(sceCLL, useCluster = "typeCluster", scale = FALSE) %>% left_join(md)
sumTab <- group_by(exprTab, condition, CLLPD, antigen) %>%
  summarise(val = median(medVal)) %>%
...
pCondi <- ggplot(annoTab, aes(x=Condition, y= "  Condition")) +
  ...
  geom_text(aes(label = Condition),size=3.2, fontface = "plain") + theme_void() +
...
pPd <- ggplot(annoTab, aes(x=colID, y= "  CLL-PD")) +
  ...
  geom_text(aes(label = `CLL-PD`),size=4, fontface = "plain") + theme_void() +
...
plot_grid(plot_grid(NULL, pColAnno,  NULL, rel_widths = c(0.47, 8, 2.32), nrow=1),
...
exprTab <- getMedian(sceTest,clusterUse) %>% left_join(md, by = "sample_id") %>%
...
#### Performing test
```{r}
resDE_condition <- diffCondiDE(sceTest, compareList, clusterUse) %>%
  ...
  plotList <- lapply(clusterPlot, function(eachCluster) {
    ...
    eachList <- lapply(compareList, function(eachPair) {
      ...
      p <- plotCyToVolcano(plotTab, fdrCut = fdrCut,
                           plotTitle = sprintf("%s vs %s",condiMap[eachPair[2]], condiMap[eachPair[1]]), useFdr=useFdr, globalPcut = globalPcut)
      p
    })
...
#ggsave(filename = "plot3Cond.pdf", height = 4.5, width = 10)
```
...
p
#ggsave(filename = "diffBoxExpr.pdf", height = 4, width = 10)
...
```{r suppl_volcano_CLLPD_CPG, fig.height = 7, fig.width = 7}
...
#ggsave("suppl_volcano_CLLPD.png", p, height = 6, width = 12)
```
...
annoColor <- list(state = c(proliferating = colList[1], `non-proliferating` = colList[2]), 
                  `Ki-67` = col_fun1,
                  `Cyclin B1` = col_fun1,
                  `P-Rb` = col_fun1)
